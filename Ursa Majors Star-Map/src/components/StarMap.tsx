...
const StarMap = () => {
  ...
  const handleMapMouseDown = (e: React.MouseEvent) => {
    ...
    // Start map dragging if not clicking on an object
    if (!draggingObjectId) {
      ...
      return (
        <div key={`sector-${sector.id}`} className="absolute inset-0" style={{ zIndex: 5 }}>
          ...
          >
            ...
            {/* Fill polygon - now with pointer events for clicking */}
            <polygon
              points={pathPoints}
              fill={gradientStyle}
              stroke={sector.color}
