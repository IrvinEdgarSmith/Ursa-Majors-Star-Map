...
interface SectorMetadataSectionProps {
  sectorId: string;
  section: MetadataSectionType;
}

const SectorMetadataSection = ({ sectorId, section }: SectorMetadataSectionProps) => {
  const { updateSectorMetadataSection, deleteSectorMetadataSection } = useStarMap();
  ...
  const handleSave = () => {
    updateSectorMetadataSection(sectorId, section.id, { heading, body });
    setIsEditing(false);
  };
  ...
  const confirmDelete = () => {
    deleteSectorMetadataSection(sectorId, section.id);
    setShowDeleteConfirmation(false);
  };

  const handleShowOnMapToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
    updateSectorMetadataSection(sectorId, section.id, { showOnMap: e.target.checked });
  };
...
export default SectorMetadataSection;
