...
import { Calendar, Copy, Pencil, Plus, Trash, X } from 'lucide-react';
...
const BoardsManager = ({ onClose }: BoardsManagerProps) => {
  ...
  const [duplicationSuccess, setDuplicationSuccess] = useState<string | null>(null);
  ...
  const handleDuplicateBoard = (boardId: string) => {
    ...
    // Set the current board as duplicating
    setDuplicatingBoardId(boardId);
    
    // Clear any previous success message
    setDuplicationSuccess(null);
    ...
    try {
      ...
      if (newBoardId) {
        ...
        setDuplicationSuccess(`Successfully duplicated "${sourceBoard?.name}"`);
        
        // Clear the duplicating state after a delay
        setTimeout(() => {
      ...
      console.error('Error during board duplication:', error);
      setDuplicatingBoardId(null);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
      ...
      >
        ...
        {duplicationSuccess && (
          <div className="mb-4 p-3 bg-green-900 bg-opacity-20 border border-green-800 rounded-md text-green-300 flex items-center gap-2">
            <Copy size={16} className="flex-shrink-0" />
            <span>{duplicationSuccess}</span>
          </div>
        )}
